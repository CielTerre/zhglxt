...
//		}
	...
	$("#yh_choose").click(function() {
			...
			layer.open({
				title:"<i class='fa fa-plus'>业务系统用户选择</i>",
				shadeClose:true,
				icon: 1,
				area: ['80%', '90%'],
				type: 1,
	...
	$("#choose_user").click(function() {
		...
		if (rows.length == 0) {
			...
			way.set("model.form.data.loginName",rows[0].YHDLZH1);
			way.set("model.form.data.name",rows[0].YHXM);
			way.set("model.form.data.password",rows[0].YHDLMM);
			way.set("model.form.data.conFirmPassword",rows[0].YHDLMM);
			way.set("model.form.data.mobile",rows[0].YHSJHM);
			way.set("model.form.data.froaid",rows[0].FROAID);
			layer.closeAll('page');
		}
	});
...
// 列表
	$('#userTable').bootstrapTable({
		url :  adminContextPath + "/user" + '/zxyhList.do',
		pagination:false,
		queryParams:function(){
			return {yhxm:way.get("model.fwsearch.yhxm")};
		},
		height:600,
		fixedScroll:true,
